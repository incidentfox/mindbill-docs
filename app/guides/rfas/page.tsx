...
export default function RfasPage() {
  return <DocPage eyebrow="Build" title="Requests for Authorization" description="Create a treatment request, review and send its signed packet, then track receipt, review, decisions, and treatment scheduling."
    ...
    <p>Capture the treatment items and diagnoses, requesting provider, claims administrator, rationale, request/review type, and any expedited-review information. React&apos;s <code>RfaDraftForm</code> provides a structured editor; the <Link href="/learn/treatment-quickstart#rfa">treatment quickstart</Link> shows its draft input. The dashboard supports initial draft creation; editing treatment rows in an existing saved draft requires your host interface and the draft API.</p>
