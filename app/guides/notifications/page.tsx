...
export default function NotificationsPage() {
  return <DocPage eyebrow="Build" title="Send useful billing notifications" description="Support users who never sign into the MindBill console, without turning a contact list into an email subscription."
    ...
    <p>These settings are available to every integration. Notifications are <strong>off by default</strong>; loading a widget or saving a billing profile never opts someone in. Angular and API-only integrations can use the same server contract with their own settings UI.</p>
    ...
    <p>Use <code>@mindbill/react@0.52.0</code> and add <code>NotificationRecipientsSettings</code> (alias <code>ConnectedNotificationRecipientsSettings</code>) to your administrator&apos;s billing settings. Enter any authorized email address, choose practice-wide or assigned-bill alerts and optional digests, and send an invitation. The recipient needs no MindBill console account or account in your app.</p>
    <Callout title="An invitation is not consent">Notifications remain off until the email owner follows the 48-hour invitation link, reviews the destination, practice scope, categories, quiet hours and digest cadence, and explicitly confirms. Opening the link alone does not enable delivery. An administrator cannot opt someone else in.</Callout>
    <p>This covers bill-status and payment alerts, 30/60/90-day aging reminders, and opt-in billing-activity digests. Each email includes an unsubscribe link. Patient reports and attachments are not emailed.</p>
    <h3>Scheduled billing-activity digests</h3>
    <p>Choose <code>reportDigest: &quot;off&quot; | &quot;daily&quot; | &quot;weekly&quot;</code>; the default is <code>off</code>. Digests are scheduled at 9 am America/Los_Angeles time, on Mondays for weekly delivery. Empty periods are skipped. They count billing updates, bills with updates and payment updates—not current statuses, financial balances or confirmed cash totals. No patient or bill details are included. Only activity since the recipient&apos;s current consent and authorized bill access is eligible.</p>
    <p>Include <code>reportDigest</code> in your server adapter&apos;s allowlist, upstream requests and preference snapshots. An existing subscription does not gain digests on upgrade: the recipient must explicitly confirm the new cadence. Existing unsubscribe and assigned-bill access rules also apply to digests.</p>
    ...
    <p>Identical retries preserve the original consent boundary, including after 24 hours. Any changed enabled PUT—including email, audience, categories, quiet hours or digest cadence—needs fresh explicit consent; changing email also needs fresh verification. Invalid updates leave the previous preference unchanged.</p>
