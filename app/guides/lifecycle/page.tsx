...
export default function LifecyclePage() {
  return (
    ...
    >
      ...
      <p>The lifecycle response includes <code>notes</code> with <code>id</code>, <code>body</code>, <code>author</code>, <code>createdAt</code>, and <code>pinned</code>. The connected component renders these notes with their author and time by default. Read the canonical bill lifecycle when switching submission attempts, so notes remain part of the bill conversation rather than a host-only copy.</p>
      <CodeBlock filename="server/add-bill-note.ts" code={`// authenticatedUser comes from your trusted host session.
await mindbill.performBillAction(billId, {
  action: "add_note",
...
// Without actorName the API uses the generic author "Partner team".`} />
      ...
      <Callout title="Notify your own application users">Use the default-off <code>NotificationSettings</code> widget with your authenticated host server to configure alerts for assigned bills or an authorized whole practice. Once enabled for your integration, the partner-recipient API supports verified, explicitly consenting users without console accounts. Signed webhooks remain available for your own content or delivery. See the <a href="/guides/notifications">notification settings and server adapter guide</a>. Courtesy-copy recipient options are not notification subscriptions.</Callout>
