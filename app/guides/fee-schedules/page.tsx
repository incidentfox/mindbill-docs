...
export default function FeeSchedulesPage() {
  return <DocPage eyebrow="Treatment billing" title="California fee calculator"
    ...
    toc={[{ id: "access", label: "Access" }, { id: "component", label: "Embed the calculator" }, { id: "imaging", label: "Imaging sessions" }, { id: "therapy", label: "Therapy bill fields" }, { id: "anesthesia", label: "Anesthesia bill fields" }, { id: "api", label: "Quote through the API" }, { id: "results", label: "Read the result" }, { id: "dates", label: "Dates and sources" }, { id: "submission", label: "Submit a treatment bill" }]}
    ...
    <CodeBlock language="bash" filename="Terminal" code="npm install @mindbill/react@0.69.7 @mindbill/browser@0.43.5" />
    ...
    <h2 id="imaging">Group imaging by the actual session</h2>
    <p>The calculator includes interpretation location and imaging session reference fields in <code>FeeScheduleCalculator</code>. Enter one encounter with all relevant imaging for the patient, physician or same provider group, and service dates, including services billed separately. Use the same opaque session reference only for services actually performed in the same session. A shared service date does not establish a shared session. Do not enter patient identifiers in the reference.</p>
    <p>The calculator treats this full encounter as complete imaging context. If a host supplies <code>completeSameDayImagingServices: false</code> through <code>initialLines</code>, that explicit incomplete context remains under review. Editing a code, date, session, location, or another line removes the entire previous quote; calculate again after completing the encounter.</p>
    <p>For direct API requests, provide actual <code>professionalComponentContext.interpretationLocation</code>, <code>imagingSessionReference</code>, and <code>completeSameDayImagingServices</code> on each relevant line, alongside claim-level <code>completeDateOfServiceContext</code>. Completeness must come from your encounter records. Session references contain 1–64 letters, digits, periods, underscores, colons, or hyphens, starting with a letter or digit. The SDK transports these facts without inferring a session or completeness.</p>
    <p>The currently supported multiple-procedure calculation covers eligible 2026 professional-component imaging with modifier <strong>26</strong>, one unit per distinct code, a physician, and the same physical interpretation location. The server checks the adopted procedure indicators, ranks each actual session, pays the highest eligible professional component at 100%, and applies 95% to subsequent eligible components, subject to the submitted charge. See <a href="https://www.dir.ca.gov/t8/9789_17_1.html">§9789.17.1, multiple-procedure payment reductions</a>.</p>
    <p>For professional-component calculations, repeated codes, mixed global or technical components, different interpretation locations, hospital outpatient caps, and unsupported clinical facts remain review cases. Use the claim-level assessment and server-returned adjustment details. <code>BillSubmissionForm</code> continues to review potentially overlapping professional-service lines; its independent line estimates are not a completed multi-line imaging allowance.</p>
    <h3>Document technical MRI services</h3>
    <p>React 0.69.6 adds technical service fields when a line has modifier <strong>TC</strong>. Record whether your billing provider or group furnished the service, whether the patient was a hospital inpatient or outpatient, the actual supervision level, and the actual imaging session reference. Empty selections stay unknown; partially entered details must be completed before requesting a quote. These facts are not inferred from the service date or place of service.</p>
    <p>Current automatic technical-component pricing is limited to distinct <strong>70551</strong>, <strong>72141</strong>, and <strong>72148</strong> services with modifier TC and one unit each, furnished by the billing physician or group in an office to a nonhospital patient, with a verified service ZIP and the reviewed July 2026 source edition. The server ranks each actual session at 100% for the highest eligible technical component and 50% for later components, subject to the submitted charge. A hospital outpatient cap must be nonbinding for this supported path. Purchased or outsourced services, hospital patients, repeated codes, mixed components, and unsupported facts remain review cases.</p>
    <p>Direct API callers supply all five fields in <code>technicalComponentContext</code>: <code>performedByBillingProviderGroup</code>, <code>patientHospitalStatus</code>, <code>supervisionLevel</code>, <code>imagingSessionReference</code>, and <code>completeSameDayImagingServices</code>. Use the claim quote endpoint even for a single TC line. The calculator includes the entered full encounter; explicit host-supplied incomplete context stays incomplete. Editing any service fact clears the entire previous quote. Read the server&apos;s ranking, reductions, source references, and final claim assessment before using an estimate.</p>
    ...
    <h3>Initial physical therapy evaluations</h3>
    <p>React 0.69.7 adds <strong>97161, 97162, and 97163</strong> to the therapy details in both <code>BillSubmissionForm</code> and <code>FeeScheduleCalculator</code>. Choose the code supported by the documented evaluation complexity. These are untimed services: do not enter treatment minutes or choose a code based on visit duration.</p>
    <p>Record whether an initial evaluation has already been billed in this treatment episode, including on earlier dates. The request uses <code>therapyContext.priorInitialEvaluationInEpisode</code>: <code>false</code> means no prior initial evaluation, <code>true</code> means one has already been billed, and an omitted value remains unknown. Unknown history and a prior initial evaluation require review. Clearing this fact removes the earlier estimate.</p>
    <p>The verified initial-evaluation calculation covers July 1 through September 30, 2026: one unit, the sole service that day, one office visit personally performed by a physical therapist, with no modifier or GP and no prior initial evaluation in the episode. The same provider, care delivery, billing arrangement, hospital status, payment-adjustment, and fee-agreement facts described above are required. Timed treatment or another same-day service remains outside this sole-service calculation.</p>
    <p>These details support the server&apos;s application of <a href="https://www.dir.ca.gov/t8/9789_15_4.html">California therapy multiple-procedure and visit rules (§9789.15.4)</a> and <a href="https://www.dir.ca.gov/t8/9789_12_9.html">physical-therapist and hospital-patient rules (§9789.12.9)</a>. The selected dated sources determine whether a service date is supported. Codes beyond the supported 97110 and initial-evaluation paths, assistant reductions, partial timed-unit allocation, and joint multi-line therapy calculations still require review.</p>
