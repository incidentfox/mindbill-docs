...
import { notificationRecipientsReact } from "@/lib/notification-recipient-recipes";
import {
...
const install = `pnpm add @mindbill/react@0.67.0`;
  ...
  return (
    <DocPage
      ...
      toc={[
        ...
        { id: "rfas", label: "Treatment and RFAs" },
    ...
    <p>Pass <code>billFromCase(caseRecord)</code> as <code>initialBill</code> and <code>finalReportSources(finalizedReportId)</code> as <code>attachments</code>. All supplied source attachments are included unless removed: do not pass every case document or rely on <code>selected: false</code>. Users can add proof of service and other supporting PDFs in the form, including a final report edited outside your app.</p>
      ...
      <p>React 0.67.0 adds patient, rendering-provider, and claims-administrator selectors backed by the complete authorized filter inventory, independent of the current page. Use canonical IDs: <code>patientId</code>, <code>renderingProviderId</code>, and <code>claimsAdministrator</code> in browser registry queries; the last serializes to <code>claimsAdminId</code> on the API. Filters never expand record access.</p>
      <p>The connected bill registry searches patient and claims-administrator names, bill and claim identifiers, external IDs, statuses, procedure codes, and dates. Every search word must match; matching ignores case. Enter text and an optional service-date or submission-date range, then press Search or Enter. Status, age, patient, rendering-provider, and claims-administrator filters apply immediately. Clear resets all filters.</p>
    ...
    <CodeBlock code={profileRecipe} language="jsx" filename="Automatic saved choices and administrator settings — React 0.62.0" />
    ...
    <Callout title="Upgrade the complete integration">This contract requires browser 0.28.0, React 0.47.0, or Angular 0.18.0 (or a later compatible release). Update any directly installed browser client alongside the component package, commit the lockfile, rebuild, and test saved SSN create, unchanged save, replace, clear, and correction flows in sandbox. A password input hides screen entry; it does not make logging or persisting the form state safe.</Callout>
      ...
      <p>From React 0.67.0, <code>ConnectedBillLifecycle</code> derives client validation and rejection field issues for editable current bills in incomplete, draft, or rejected states. An explicit <code>validationIssues</code> value overrides that default. Other states do not gain draft validation errors.</p>
      ...
      <p>Pass <code>onPatientClick(patient)</code>, <code>onRenderingProviderClick(provider)</code>, and <code>onClaimsAdministratorClick(administrator)</code> to <code>BillReadOnlyForm</code> or <code>ConnectedBillLifecycle</code> to make those names open your own related-record view. IDs are optional on historical snapshots: check for a canonical ID before applying a filter, and do not infer identity from a name. On these standalone detail components, names remain plain text without callbacks. In React 0.67.0, <code>ConnectedBillingWorkspace</code> supplies default callbacks that open All Bills filtered by the canonical entity ID and reset prior filters and pagination. Override the corresponding workspace callback to use your host navigation. Historical entities without IDs remain plain text; names are never used as guessed IDs. These callbacks do not change the bill&apos;s saved data or grant access to another record.</p>
